...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        # if int(last_frame_id) + cfg.DATA_CONFIG.FRAME_STRIDE != int(batch_dict['token']['this_sample_idx'][0]):
        #     batch_dict['token']['is_first_frame'] = True
        #     history_features = None
        # else:
        #     batch_dict['token']['is_first_frame'] = False
        # if history_features is not None:
        #     batch_dict['token']['history_features'] = history_features

        with torch.no_grad():
            pred_dicts, ret_dict = model(batch_dict)
        # history_features = ret_dict.get('history_features', None)
        # last_frame_id = copy.deepcopy(batch_dict['token']['this_sample_idx'][0])
    ...
    if det_annos and 'gt_boxes' in batch_dict['token']:
