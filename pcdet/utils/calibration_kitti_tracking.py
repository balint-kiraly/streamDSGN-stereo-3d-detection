...
def get_calib_from_file(calib_file):
    ...
    calib_data = {}
    for key in ['P0', 'P1', 'P2', 'P3', 'R_rect', 'Tr_velo_cam']:
        for line in lines:
            line = line.strip()
            splits = [x for x in line.split(' ') if len(x.strip()) > 0]
            if splits[0][:] == key or splits[0][:-1] == key:
                obj = splits[1:]
    ...
    return {'P0': calib_data['P0'].reshape(3, 4),
            ...
            'R0': calib_data['R_rect'].reshape(3, 3),
            'Tr_velo2cam': calib_data['Tr_velo_cam'].reshape(3, 4)}
...
class Calibration(object):
    ...
    def image_scale(self, scale_factor):
         # is that true?
        self.P2[:2, :3] *= scale_factor
        self.P3[:2, :3] *= scale_factor
...
if __name__ == '__main__':
    ...
    # __workdir__ = Path(__file__).parent.parent.parent.resolve()
    # sys.path.insert(0, str(__workdir__))
    calib = Calibration('./data/kitti_tracking/training/calib/0000.txt')
