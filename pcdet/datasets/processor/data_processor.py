...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        ...
        if 'completion_points' in data_dict:
            mask = common_utils.mask_points_by_range(
                data_dict['completion_points'], self.point_cloud_range)
    ...
    def mask_stream_points_and_boxes_outside_range(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.mask_stream_points_and_boxes_outside_range, config=config)
        mask = common_utils.mask_points_by_range(
            data_dict['input_data']['points'], self.point_cloud_range)
        data_dict['input_data']['points'] = data_dict['input_data']['points'][mask]
        if 'completion_points' in data_dict['input_data']:
            mask = common_utils.mask_points_by_range(
                data_dict['input_data']['completion_points'], self.point_cloud_range)
            data_dict['input_data']['completion_points'] = data_dict['input_data']['completion_points'][mask]

        if data_dict['gt_info'].get('gt_boxes', None) is not None and config.REMOVE_OUTSIDE_BOXES and self.training:
            mask = box_utils.mask_boxes_outside_range_numpy(
                data_dict['gt_info']['gt_boxes'], self.point_cloud_range, min_num_corners=config.get(
                    'min_num_corners', 1)
            )
            data_dict['gt_info']['gt_boxes'] = data_dict['gt_info']['gt_boxes'][mask]
            for key in ['gt_names', 'gt_truncated', 'gt_occluded', 'gt_difficulty', 'gt_index', 'gt_boxes_no3daug', 'object_id']:
                if key in data_dict['gt_info']:
                    data_dict['gt_info'][key] = data_dict['gt_info'][key][mask]
        return data_dict
    ...
    def shuffle_stream_points(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.shuffle_stream_points, config=config)

        if config.SHUFFLE_ENABLED[self.mode]:
            points = data_dict['input_data']['points']
            shuffle_idx = np.random.permutation(points.shape[0])
            points = points[shuffle_idx]
            data_dict['input_data']['points'] = points
    ...
    def transform_points_to_projected_voxels(self, data_dict=None, config=None):
        ...
        points = data_dict.get('completion_points', data_dict.get(
            'points_no3daug', data_dict['points']))
        ...
        voxel_sizes = np.asarray(
            [self.VOXEL_X_SIZE, self.VOXEL_Y_SIZE, self.VOXEL_Z_SIZE])[None]
        image_shape = (data_dict['left_img'].shape[:2]
                       if 'left_img' in data_dict else data_dict['left_imgs'].shape[1:3])
        voxelized_points = ((points[:, :3] - np.asarray([self.X_MIN, self.Y_MIN, self.Z_MIN])) / voxel_sizes).astype(int) * voxel_sizes + \
            ...
            np.asarray([self.X_MIN, self.Y_MIN, self.Z_MIN]
                       )  # ensure voxelized to grid centers
        rect_voxelized_points = Calibration.lidar_pseudo_to_rect(
            voxelized_points[:, :3])
        voxelized_depth_gt_img, voxelized_deviation_gt_img = depth_map_utils.points_to_depth_deviation_map(
            rect_voxelized_points, image_shape, data_dict['calib'])
        ...
        iy, ix = np.round(pts_img[:, 1]).astype(
            np.int64), np.round(pts_img[:, 0]).astype(np.int64)

        voxels_in_ray = (iy >= 0) & (ix >= 0) & (
            iy < voxelized_depth_gt_img.shape[0]) & (ix < voxelized_depth_gt_img.shape[1])
        voxels_in_ray = np.where(voxels_in_ray)[0]

        valid_voxel = voxelized_depth_gt_img[iy[voxels_in_ray],
                                             ix[voxels_in_ray]] > 0.00001
        ...
        if config.get('POS_WEIGHT', False):
            data_dict['norm_dist'] = np.linalg.norm(
                rect_points[voxels_in_ray], axis=1)

        data_dict['voxels_in_ray'] = voxels_in_ray
        data_dict['occupany_of_voxels_in_ray'] = valid_occupancy.astype(
            np.float32)
    ...
    def transform_stream_points_to_voxels(self, data_dict=None, config=None, voxel_generator=None):
        if data_dict is None:
            try:
                from spconv.utils import VoxelGeneratorV2 as VoxelGenerator
            except:
            ...
            self.grid_size = np.round(grid_size).astype(np.int64)
            self.voxel_size = config.VOXEL_SIZE
            return partial(self.transform_stream_points_to_voxels, voxel_generator=voxel_generator)
        ...
        if not data_dict['input_data']['use_lead_xyz']:
            voxels = voxels[..., 3:]  # remove xyz in voxels(N, 3)

        data_dict['input_data']['voxels'] = voxels
        data_dict['input_data']['voxel_coords'] = coordinates
        data_dict['input_data']['voxel_num_points'] = num_points
        data_dict['input_data']['voxel_size'] = self.voxel_size
        return data_dict
