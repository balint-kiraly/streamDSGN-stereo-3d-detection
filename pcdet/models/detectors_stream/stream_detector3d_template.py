...
from pcdet.models import backbones_2d, backbones_3d_stream, dense_heads, depth_heads, fusion_module
...
class StreamDetector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.use_amp_dict = model_cfg.get('USE_AMP', {'TRAIN':False, 'TEST':False})
        self.with_grad_when_obtaining_history = model_cfg.get('WITH_GRAD_WHEN_OBTAINING_HISTORY', False)
        self.register_buffer('global_step', torch.LongTensor(1).zero_())

        self.module_topology = [
            'backbone_3d', 'map_to_bev_module', 'spatial_feature_fusion_module',
            'backbone_2d', 'spatial_feature_2d_fusion_module', 'dense_head_2d', 'dense_head',
        ...
        if 'DENSE_HEAD' in self.model_cfg and getattr(self.model_cfg.DENSE_HEAD, 'do_feature_imitation', False):
            if hasattr(self.model_cfg, 'TEACHER_MODEL'):
                self.module_topology.insert(0, 'teacher_model')
            # else:
            #     self.module_topology.insert(0, 'student_model')
    ...
    def build_networks(self):
        model_info_dict = {
            'module_list': [],
            'fixed_module_list': [],
            'use_amp_dict': self.use_amp_dict,
        ...
        if self.model_cfg.get('FREEZE_RPN', False):
            for module_name in self.module_topology:
                ...
                if module is None or module_name == 'roi_head':
                    continue
    ...
    def build_lidar_model(self, model_info_dict):
        ...
        lidar_model = build_lidar_detector(
            self.model_cfg.LIDAR_MODEL, self.num_class, self.dataset)
    ...
    def build_backbone_3d(self, model_info_dict):
        ...
        backbone_3d_module = backbones_3d_stream.__all__[self.model_cfg.BACKBONE_3D.NAME](
            ...
            use_amp_dict=model_info_dict['use_amp_dict']
            )
    ...
    def build_spatial_feature_fusion_module(self, model_info_dict):
        if self.model_cfg.get('FUSION_IN_SPATIAL_FEATURES', None) is None:
            return None, model_info_dict

        spatial_feature_fusion_module = fusion_module.__all__[self.model_cfg.FUSION_IN_SPATIAL_FEATURES.NAME](
            model_cfg=self.model_cfg.FUSION_IN_SPATIAL_FEATURES,
            input_channels=model_info_dict['num_bev_features'],
        )
        model_info_dict['module_list'].append(spatial_feature_fusion_module)
        model_info_dict['num_bev_features'] = spatial_feature_fusion_module.num_bev_features
        return spatial_feature_fusion_module, model_info_dict
    ...
    def build_spatial_feature_2d_fusion_module(self, model_info_dict):
        if self.model_cfg.get('FUSION_IN_SPATIAL_FEATURES_2D', None) is None:
            return None, model_info_dict

        spatial_feature_2d_fusion_module = fusion_module.__all__[self.model_cfg.FUSION_IN_SPATIAL_FEATURES_2D.NAME](
            model_cfg=self.model_cfg.FUSION_IN_SPATIAL_FEATURES_2D,
            input_channels=model_info_dict['num_bev_features'],
        )
        model_info_dict['module_list'].append(spatial_feature_2d_fusion_module)
        model_info_dict['num_bev_features'] = spatial_feature_2d_fusion_module.num_bev_features
        return spatial_feature_2d_fusion_module, model_info_dict

    def build_dense_head_2d(self, model_info_dict):
        if self.model_cfg.get('DENSE_HEAD_2D', None) is None:
            return None, model_info_dict
        if self.model_cfg.DENSE_HEAD_2D.NAME in ['MMDet2DHead', 'StreamMMDet2DHead']:
    ...
    def build_dense_head(self, model_info_dict):
        ...
        dense_head_module = dense_heads.__all__[self.model_cfg.DENSE_HEAD.NAME](
            ...
            point_cloud_range=model_info_dict['point_cloud_range'],
            predict_boxes_when_training=self.model_cfg.get(
                'ROI_HEAD', False) or self.model_cfg.DENSE_HEAD.get('predict_boxes_when_training', False)
        )
        model_info_dict['module_list'].append(dense_head_module)
        return dense_head_module, model_info_dict
    ...
    def build_depth_loss_head(self, model_info_dict):
        assert not self.model_cfg.BACKBONE_3D.get('front_surface_depth', False) or \
            (self.model_cfg.BACKBONE_3D.get('drop_psv_loss', False)
             or self.model_cfg.BACKBONE_3D.get('drop_psv', False))
        if (self.model_cfg.get('DEPTH_LOSS_HEAD', None) is None or self.model_cfg.BACKBONE_3D.get('drop_psv', False)) and \
            ...
            return None, model_info_dict
        depth_loss_head = depth_heads.__all__[self.model_cfg.DEPTH_LOSS_HEAD.NAME](
            model_cfg=self.model_cfg.DEPTH_LOSS_HEAD,
            point_cloud_range=getattr(
                model_info_dict, 'stereo_point_cloud_range', model_info_dict['point_cloud_range']),
        )
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                if not isinstance(cls_preds, list):
                    ...
                    multihead_label_mapping = [torch.arange(
                        1, self.num_class + 1, device=cls_preds[0].device)]
                else:
                ...
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    ...
                    cur_label_preds = label_preds[cur_start_idx: cur_start_idx +
                                                  cur_cls_preds.shape[0]] if label_preds is not None else None
            ...
            mask_invalid = torch.isnan(final_boxes).any(dim=1) | torch.isinf(final_boxes).any(dim=1)
            final_scores = final_scores[~mask_invalid]
            final_labels = final_labels[~mask_invalid]
            final_boxes = final_boxes[~mask_invalid]
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        if cur_gt.shape[0] > 0:
            ...
            if iou3d_rcnn.shape[0] == 0:
                ...
                ioubev_results = ioubev_rcnn.max(
                    0).values.cpu().numpy().tolist()
