...
class StreamDepthLossHead(nn.Module):
    def __init__(self, model_cfg, point_cloud_range):
        ...
        self.depth_supervision = model_cfg.get('DEPTH_SUPERVISION', 'token')
        self.get_depth_error_map_when_inference = model_cfg.get('GET_DEPTH_ERROR_MAP_WHEN_INFERENCE', False)

    def get_loss(self, batch_dict, tb_dict=None):
        if tb_dict is None:
            tb_dict = {}

        depth_preds = batch_dict['token']['depth_preds']
        depth_volumes = batch_dict['token']['depth_volumes']
        depth_sample = batch_dict['token']['depth_samples']
        gt = batch_dict[self.depth_supervision]['depth_gt_img'].squeeze(1)
        height = gt.shape[1]
    ...
    def forward(self, batch_dict):
        ...
        if not self.training and self.get_depth_error_map_when_inference:
            # depth_pred = batch_dict['depth_preds'][-1]
            this_data_dict = batch_dict['token']
            depth_pred_locals = this_data_dict['depth_preds_local'][-1]
            ...
            this_data_dict['depth_error_all_local_median'] = []
            for thresh in [0.2, 0.4, 0.8, 1.6]:
                this_data_dict[f"depth_error_all_local_{thresh:.1f}m"] = []
            this_data_dict['depth_error_fg_local_statistics_perbox'] = []

            for b in range(N):
                #TODO(hack)
                depth_pred_local = depth_pred_locals[..., :this_data_dict['depth_gt_img'].shape[-1]][b:b+1]
                gt = this_data_dict['depth_gt_img'].squeeze(1)[b:b+1]
                depth_fgmask_img = this_data_dict['depth_fgmask_img'].squeeze(1)[b:b+1]
                ...
                this_data_dict['depth_error_all_local_median'].append( median_error )
                for thresh in [0.2, 0.4, 0.8, 1.6]:
                    this_data_dict[f"depth_error_all_local_{thresh:.1f}m"].append( (error_map[mask] > thresh).float().mean() )

                if 'depth_fgmask_img' in this_data_dict:
                    ...
                    fg_gts = gt[fg_mask]
                    this_data_dict['depth_error_fg_local_statistics_perbox'].append( [] )
                    fg_ids = depth_fgmask_img[fg_mask].int() - 1
                    if len(fg_ids) > 0:
                        for idx in range(fg_ids.min().item(), fg_ids.max().item() + 1):
                            if this_data_dict['gt_index'][b][idx] < 0:
                                continue
                            ...
                            this_data_dict['depth_error_fg_local_statistics_perbox'][-1].append(dict(
                                distance=fg_gt_i_median,
                                err_median=errs_i.median().item(),
                                num_points=num_points_i,
                                name=this_data_dict['gt_names'][b][idx],
                                truncated=this_data_dict['gt_truncated'][b][idx],
                                occluded=this_data_dict['gt_occluded'][b][idx],
                                difficulty=this_data_dict['gt_difficulty'][b][idx],
                                index=this_data_dict['gt_index'][b][idx],
                                idx=idx,
                                image_idx=this_data_dict['this_sample_idx'][b]
                            ))

                            for thresh in [0.2, 0.4, 0.8, 1.6]:
                                this_data_dict['depth_error_fg_local_statistics_perbox'][-1][-1][f"err_{thresh:.1f}m"] = (errs_i > thresh).float().mean().item()
