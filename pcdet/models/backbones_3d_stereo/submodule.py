...
class hourglass2d(nn.Module):
    def __init__(self, inplanes, gn=False, model_cfg=None):
...
class hourglass2d_large_kernel(nn.Module):
    def __init__(self, inplanes, gn=False, model_cfg=None):
        super(hourglass2d_large_kernel, self).__init__()

        kernel_list = [3, 3, 3, 3] if model_cfg is None else model_cfg.get('kernel_list', [3, 3, 3, 3])
        stride_list = [2, 1, 2, 1] if model_cfg is None else model_cfg.get('stride_list', [2, 1, 2, 1])
        padding_list = [1, 1, 1, 1] if model_cfg is None else model_cfg.get('padding_list', [1, 1, 1, 1])
        dilation_list = [1, 1, 1, 1] if model_cfg is None else model_cfg.get('dilation_list', [1, 1, 1, 1])
        ...
        self.conv2 = convbn(inplanes * 2,
                            ...
                            gn=gn,
                            groups_conv=inplanes*2)
        self.conv2_1 = nn.Conv2d(inplanes * 2, inplanes * 2, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.conv4 = nn.Sequential(
            convbn(inplanes * 2,
                   ...
                   gn=gn,
                   groups_conv=inplanes*2), nn.ReLU(inplace=True))
        
        self.conv4_1 = nn.Conv2d(inplanes * 2, inplanes * 2, kernel_size=1, stride=1, padding=0, bias=False)
