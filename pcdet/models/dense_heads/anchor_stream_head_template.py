...
class AnchorStreamHeadTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, predict_boxes_when_training):
        ...
        self.reduce_avg_factor = getattr(
            self.model_cfg, 'reduce_avg_factor', True)
        ...
        self.do_feature_imitation = getattr(
            model_cfg, 'do_feature_imitation', False)
        if self.do_feature_imitation:
            cfgs = model_cfg.imitation_cfg if isinstance(
                model_cfg.imitation_cfg, list) else [model_cfg.imitation_cfg]
            ...
            for cfg in cfgs:
                ...
                elif cfg.layer == "identity":
                    layers.append(nn.Identity())
                else:
                    assert cfg.layer == "none", f"invalid layer type {cfg.layer}"
                if cfg.get('use_relu', False):
                    layers.append(nn.ReLU())
                    assert cfg.normalize is None

                if cfg.get('normalize', False):
                    self.norm_imitation[cfg.student_feature_layer] = NormalizeLayer(
                        cfg.normalize, cfg.channel)
                else:
                    self.norm_imitation[cfg.student_feature_layer] = nn.Identity(
                    )
    ...
    def build_losses(self, losses_cfg):
        ...
        self.add_module(
            'imitation_loss_func',
            getattr(loss_utils, losses_cfg.get(
        ...
        if losses_cfg.get('TREND_LOSS_TYPE', None) is not None:
            self.add_module(
                'trend_loss_func',
                getattr(loss_utils, losses_cfg.get('TREND_LOSS_TYPE', 'WeightedSmoothL1Loss'))(
                    code_weights=losses_cfg.LOSS_WEIGHTS['trend_code_weight'])
            )
        self.add_module(
            'dir_loss_func',
    ...
    def get_cls_layer_loss(self):
        ...
        if 'cam_centerness' in self.forward_ret_dict:
            if True:
                cls_weights = (negative_cls_weights + 1.0 *
                               positives).float().detach()
                ...
                pos_normalizer = positives.sum(
                    1, keepdim=True).float().detach()
            ...
            one_hot_targets.scatter_(-1, cls_targets.unsqueeze(dim=-
                                     1).long(), cam_centerness[..., None].float())

            cls_preds = cls_preds.view(batch_size, -1, self.num_class)
            one_hot_targets = one_hot_targets[..., 1:]
            print(
                f'{clone_one_hot_targets.sum(dim=1)} -> {one_hot_targets.sum(dim=1)}')
    ...
    def get_direction_target(anchors, reg_targets, one_hot=True, dir_offset=0, num_bins=2):
        ...
        offset_rot = common_utils.limit_period(
            rot_gt - dir_offset, 0, 2 * np.pi)
        dir_cls_targets = torch.floor(
            offset_rot / (2 * np.pi / num_bins)).long()
        dir_cls_targets = torch.clamp(dir_cls_targets, min=0, max=num_bins - 1)

        if one_hot:
            dir_targets = torch.zeros(*list(dir_cls_targets.shape), num_bins, dtype=anchors.dtype,
                                      device=dir_cls_targets.device)
            dir_targets.scatter_(-1,
                                 dir_cls_targets.unsqueeze(dim=-1).long(), 1.0)
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_preds_sin, reg_targets_sin = self.box_coder.process_before_loss(
            anchors[pos_inds], box_preds[pos_inds], box_reg_targets[pos_inds])
        loc_loss_src = self.reg_loss_func(
            box_preds_sin, reg_targets_sin, weights=reg_weights[pos_inds])  # [N, M]
        ...
        iou_loss_weight = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS.get(
            'iou_weight', 0.)
        if iou_loss_weight > 0:
            ...
            iou_loss_src = self.iou_loss_func(
                decoded_box_preds, decoded_reg_targets, weights=reg_weights[pos_inds])
        ...
        if box_dir_cls_preds is not None:
            ...
            if self.reduce_avg_factor:
                dir_weight_normalizer = dist_reduce_mean(
                    dir_weight_normalizer.view(-1).mean())
            dir_weight_normalizer = torch.clamp(
                dir_weight_normalizer, min=self.clamp_value)
    ...
    def get_imitation_reg_layer_loss(self, features_preds, features_targets, imitation_cfg):
        ...
        if imitation_cfg["mode"] == "inbox":
            ...
            anchors_xyz[..., 2] = 0
            gt_boxes[..., 2] = 0
            positives = points_in_boxes_gpu(anchors_xyz.view(
                anchors_xyz.shape[0], -1, 3), gt_boxes)
        ...
        positives = torch.logical_and(
            positives, torch.any(features_targets != 0, dim=-1))
        ...
        pos_feature_targets = self.norm_imitation[imitation_cfg.student_feature_layer](
        ...
        imitation_loss = imitation_loss * \
            self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['imitation_weight'] * imitation_cfg.get(
                'weight', 1.0)
        ...
        if pos_inds.sum() > 0:
            rel_err = torch.median(
                torch.abs((pos_feature_preds - pos_feature_targets) / pos_feature_targets))
            rel_err_mean = torch.mean(
                torch.abs((pos_feature_preds - pos_feature_targets) / pos_feature_targets))
