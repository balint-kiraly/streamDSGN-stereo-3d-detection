...
class MMDet2DHead(nn.Module):
    ...
    def get_loss(self, data_dict, tb_dict):
        ...
        gt_boxes_2d, gt_labels = torch.split(
            data_dict['gt_boxes_2d'], (4, 1), dim=2)
        gt_labels = gt_labels[..., 0].long()
        if self.use_3d_center:
            gt_boxes_2d = torch.cat(
                [gt_boxes_2d, data_dict['gt_centers_2d']], dim=-1)
        ...
        gt_bboxes_2d_ignore = torch.unbind(
            data_dict['gt_boxes_2d_ignored']) if 'gt_boxes_2d_ignored' in data_dict else None  # a list of [N, 4] tensors
